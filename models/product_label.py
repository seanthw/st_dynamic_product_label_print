...
class ProductLabelWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        default_paperformat_id = get_param(
            "st_dynamic_product_label_print.paperformat_id"
        )
        if default_paperformat_id:
            res["paperformat_id"] = int(default_paperformat_id)

        # FORCE 3x10 Default as per user requirement, ignoring stored history for now
        res["print_format"] = "3x10"
        res["rows"] = 10
        res["cols"] = 3

        res["vertical_spacing"] = float(
            get_param("st_dynamic_product_label_print.label_vertical_spacing", 1.0)
        )
        res["horizontal_spacing"] = float(
            get_param("st_dynamic_product_label_print.label_horizontal_spacing", 1.0)
        )

        res["show_barcode"] = (
            get_param("st_dynamic_product_label_print.label_show_barcode") == "True"
        )
        res["show_barcode_digits"] = (
            get_param("st_dynamic_product_label_print.label_show_barcode_digits")
            == "True"
        )
        res["show_internal_ref"] = (
            get_param("st_dynamic_product_label_print.label_show_internal_ref")
            == "True"
        )
        res["show_on_hand_qty"] = (
            get_param("st_dynamic_product_label_print.label_show_on_hand_qty") == "True"
        )
        res["show_stock_label"] = (
            get_param("st_dynamic_product_label_print.label_show_stock_label") == "True"
        )
        res["show_attributes"] = (
            get_param("st_dynamic_product_label_print.label_show_attributes") == "True"
        )
        res["show_attribute_key"] = (
            get_param("st_dynamic_product_label_print.label_show_attribute_key")
            == "True"
        )
    ...
    show_barcode = fields.Boolean(
        ...
        .sudo()
        .get_param("st_dynamic_product_label_print.label_show_barcode")
        == "True",
    )
    ...
    show_on_hand_qty = fields.Boolean(
        ...
        default=lambda self: self.env["ir.config_parameter"]
        .sudo()
        .get_param("st_dynamic_product_label_print.label_show_on_hand_qty")
        == "True",
    )
    show_stock_label = fields.Boolean(
        ...
        default=lambda self: self.env["ir.config_parameter"]
        .sudo()
        .get_param("st_dynamic_product_label_print.label_show_stock_label")
        == "True",
    )
    show_attributes = fields.Boolean(
        ...
        default=lambda self: self.env["ir.config_parameter"]
        .sudo()
        .get_param("st_dynamic_product_label_print.label_show_attributes")
        == "True",
    )
    ...
    def _get_config_params(self):
        ...
        return {
            ...
            == "True",
            "show_attribute_key": get_param(
                "st_dynamic_product_label_print.label_show_attribute_key"
            )
            == "True",
        }
    ...
    def _calculate_dynamic_styles(
        self, base_font_size, product_name, attribute_string, show_attributes
    ):
        """
    ...
    def action_print_labels(self):
        # FORCE overrides to ensure correct layout regardless of UI state
        self.rows = 10
        self.cols = 3
        ...
        paperformat = self.paperformat_id or self.env.ref(
            "st_dynamic_product_label_print.paperformat_label", raise_if_not_found=False
        )
        if not paperformat:
            raise UserError(
                _(
                    "Paper format is not configured. Please set a default in settings or select one in the wizard."
                )
            )
        ...
        temp_paperformat = paperformat.copy(
            {
                ...
                "page_height": 279.4,  # US Letter Height
                "page_width": 215.9,   # US Letter Width
                "format": "custom",
            }
        )
        ...
        if all_labels_flat:  # Only pad if there are labels/placeholders to process
            ...
            initial_offset > 0
        ):  # If no actual labels but initial offset, ensure placeholders exist
        ...
        data = {
            ...
            **config,
            ...
            "label_height": config[
                "label_height"
            ],  # Pass label_height for fixed-height placeholders
        }
